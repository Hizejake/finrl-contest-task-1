...
SELECTED_MODEL = "gemma"  # Options: gemma, llama3, deepseek
...
NEWS_CSV_PATH = "news_train.csv"
LOB_DATA_PATH = "BTC_1min.csv"
OUTPUT_DIR = "datasets"
...
if model is not None and tokenizer is not None:
    ...
    if news_df.empty:
        ...
        news_for_merge = pd.DataFrame(
            {'system_time': pd.Series(dtype='datetime64[ns, UTC]'), 'sentiment_score': pd.Series(dtype='float64'), 'risk_score': pd.Series(dtype='float64')}
        )
    else:
